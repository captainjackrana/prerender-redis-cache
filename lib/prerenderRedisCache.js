...
var lines = fs.readFileSync(process.env.CACHE_WHITELIST_PATH || 'cache_whitelist.txt', 'UTF-8').split(/\r?\n/);
// print all lines
lines.forEach(line => {
    if(line){
        whitelistedUrls[url.parse(line.trim()).pathname] = 1;
    }
});
console.log('Loaded prerender cache whitelist');
...
module.exports = {
    requestReceived: function (req, res, next) {
        ...
        client.get(req.prerender.url + (req.prerender.height == '667' ?':m': ''), function (error, result) {
            if (!error && result) {
    ...
    pageLoaded: function (req, res, next) {
        ...
        var key = req.prerender.url + (req.prerender.height == '667' ?':m': '');
        var response = {
            statusCode: req.prerender.statusCode,
            content: req.prerender.content.toString(),
            headers: req.prerender.headers
        };
        if(whitelistedUrls[url.parse(req.prerender.url).pathname]){
